...
> 本文面向第一次接触工业相机的视觉组成员，重点是**相机参数的调节，以及 MVS 和 SDK 的使用**。
...
镜头上的**光圈环** 就是用来手动转动、调节进光量的。光圈还影响景深：大光圈景深浅（背景容易虚化），小光圈景深大。
...
| **外触发（硬件触发）** | 外部 I/O 收到脉冲信号才曝光 | 对齐时间戳 |
...
厂商为什么要给你 SDK？因为工业相机底层涉及 USB/网口协议、寄存器读写、图像缓冲管理，这些不该让每个用户自己写。厂商把底层封装好，对外暴露一组**函数（API）**，再附上**文档** 和**示例代码**，就组成了一个 SDK。

**关键认识**：

1. **头文件（Include）** 告诉你有哪些函数可以调、参数是什么；
2. **库（Libraries）** 才是函数真正的实现，编译和运行时必须链接到它；
3. **示例（Samples）** 是官方写好的最小可用代码，学习顺序是**先跑通示例，再改示例，最后才自己写**；

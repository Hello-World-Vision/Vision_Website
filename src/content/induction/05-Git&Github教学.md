...
> 本文档讲的是版本控制。
> 写视觉代码，光把文件存在文件夹里是不够的。代码要能回退到之前能跑的版本，也要方便多人协作、查清谁在什么时候改了哪里，这些都靠 Git 来管理。
...
按 Enter 会把密钥保存在默认路径，也可以在提示时为密钥设置密码。
...
GitHub 仓库分为 Public（公开）和 Private（私有）两种类型，区别在于代码的可见范围。

*   **Public（公开仓库）**：所有人可见，任何人都能在互联网上搜索、查看和下载你的代码，但只有你授权的人才能提交修改。适合开源项目、分享工具，或用来展示自己的编程能力。
*   **Private（私有仓库）**：只有你以及你邀请的协作者（Collaborators）才能查看和访问，适合商业项目、公司代码、尚未完成的草稿，或包含敏感信息的个人备份。

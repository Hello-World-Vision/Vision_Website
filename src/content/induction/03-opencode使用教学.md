...
Agent 给出的代码只是候选方案，不是标准答案。它不知道你的相机参数、你的底盘动力学、你的赛场环境。你必须逐行理解它给出的每一段逻辑，必须能向任何一名队员解释你的代码在做什么、为什么这样做、在什么情况下会失效。
...
安装前确认 Node.js（[https://nodejs.org/](https://nodejs.org/)）或 Bun（[https://bun.sh/](https://bun.sh/)）已安装并可用。以下两种安装方式二选一，不可同时使用。
...
## 4. UI 界面与交互规范（参考官方实战仓库）

> 本节标题沿用培训大纲中的"官方实战仓库"表述。实际指定的 `ForceInjection/opencode-practise` 是第三方实践仓库，不是 OpenCode 官方仓库，不具备规范解释权。
...
角色切换不是装饰性操作。不同 Agent 的职责、工具和权限可能不同。执行任务前必须确认当前模式是否与目标一致。
...
按以下顺序阅读，逐篇操作。每篇读完、操作完，再进入下一篇：

1. **OpenCode 官方配置文档（基础）**
   https://opencode.ai/docs/zh-cn/config/
   作用：理解 `opencode.json` 的结构、配置项含义、provider 与 model 的关系、Schema 校验规则。
   指令：通读一遍。配置相关的所有疑问，优先查阅此文档。

2. **菜鸟教程：OpenCode 接入 DeepSeek（实操）**
   https://m.runoob.com/opencode/opencode-deepseek.html
   作用：从申请 API Key 到写入配置文件到启动验证，完整的端到端操作流程。
   指令：跟随教程逐步操作。如果教程中的命令与当前客户端行为不一致，以客户端实际提示为准。

3. **CSDN：OpenCode 安装与配置详解（可以直接操作）**
   https://blog.csdn.net/qq_43462019/article/details/157034844
   作用：补充环境变量设置、常见报错排查、多 Provider 共存等实际场景。
   指令：如上述两篇无法解决当前问题，定向检索对应章节。不要通篇阅读。

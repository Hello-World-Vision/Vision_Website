...
<a id="答案"></a>
> [!NOTE] 解答
> Linux中权限通常写成`rwx`的形式，分别对应读、写、执行，三组`rwx`则分别对应所有者、所属组和其他用户。我们在输入`chmod +x`后本质上是为`try.sh`添加了执行权限，使其从`rw-rw-r--`变为`-rwxrwxr-x`。此外，`rwx`的三位分别对应二进制中的三位数字，因此我们也常会看到`chmod 777 try.sh`的做法，读者可以自己尝试和理解。

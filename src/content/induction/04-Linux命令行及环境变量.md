...
对于常年使用 Windows 图形界面的新人来说，一开始面对 Linux 下频繁的终端操作时往往会感到无所适从。在第1章中，我将带你理解什么是命令行，学习常见的命令，并教你如何借助 LLM（大语言模型）和终端 Agent 来辅助操作。
...
*   **错误做法**：凭着模糊的记忆乱敲 `tar -zxvf`，或者去搜索引擎翻找 5 年前的垃圾博客。(对于你知道应该使用什么命令只是忘了使用格式时，这样的查找是可取的)
*   **正确做法**：直接在终端呼出 Agent，或者打开你的大模型对话框输入：*"我下载了一个 xxx.tar.gz 文件，帮我写一段 Ubuntu 命令把它解压到 /opt 目录下，并用大白话解释每个参数的意思。"*

> [!TIP] **小工具** 
> 如果你很想知道命令到底做了什么，把你的命令输到[这个网站](https://explainshell.com/),它会提供详尽的英文解释。
---
...
### 2.1 权限控制（Permission）
Linux 默认不允许普通用户直接触碰底层硬件和系统核心文件。比如，当你执行一个没有执行权限的脚本时，终端就会报错。下面是一个例子
我们试着创建一个脚本`try.sh`
```bash
#!/bin/bash
...
./try.sh
```
你会发现终端没有输出`Hello, this is try.sh!`，而是输出了这一行报错：
```bash
bash: ./try.sh: Permission denied
```
显然，权限不够。你可以先把这个报错扔给AI,试着自己解决。
以下是我的做法：
输入`ls -l`,找到输出中的`try.sh`
```bash
-rw-rw-r--  1 walt walt   43  9月  3 12:47 try.sh
```
这里的`-rw-rw-r--`正是问题所在，其含义我放在[下面](#答案)解答
改变文件权限的命令是`chmod`，我们输入以下命令
```bash
chmod +x try.sh
```
再`ls -l`,就会发现`try.sh`的属性变成了这样
```bash
-rwxrwxr-x  1 walt walt   43  9月  3 12:47 try.sh
```
此时我们`./try.sh`执行，就可以成功输出`Hello, this is try.sh!`。
<a id="答案"></a>
> [!NOTE] **解答** 
> Linux中权限通常写成`rwx`的形式，分别对应读、写、执行，三组`rwx`则分别对应所有者、所属组和其他用户。我们在输入`chmod +x`后本质上是为`try.sh`添加了执行权限，使其从`rw-rw-r--`变为`-rwxrwxr-x`。此外，`rwx`的三位分别对应二进制中的三位数字，因此我们也常会看到`chmod 777 try.sh`的做法，读者可以自己尝试和理解。此外，你会发现即使不给`try.sh`执行权限，通过`bash try.sh`也能成功执行，为什么呢？它用的是什么权限？



> [!TIP]**💡 学会向 LLM 提问**
>  如果你的命令出现了报错，你不知道要如何处理，或者不确定自己的处理是否正确，你应该把报错信息扔给 AI。
> **你的 Prompt 示范**：*"我在 Ubuntu 下运行 C++ 视觉程序，尝试读取 /dev/ttyUSB0 串口时报错 `Permission Denied`。我已经确认硬件插好了，告诉我可能是什么权限问题？如何解决？"*
...
> 我写这些让 Agent 辅助操作的目的就是希望你在有一定命令行基础的情况下，有能力慢慢学会使用Agent去解决很多问题。
---
...
*   **真相**：你每次新打开一个终端，系统在显示光标前，都会把 `~/.bashrc` 里的所有代码执行一遍。
